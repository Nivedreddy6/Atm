...
# Vercel Serverless Function Support (Read-only filesystem workaround)
if os.environ.get("VERCEL"):
    import shutil
    vercel_db_path = "/tmp/atm.db"
    if not os.path.exists(vercel_db_path):
        try:
            shutil.copy2(DB_PATH, vercel_db_path)
            print("Copied database to /tmp for write-access on Vercel.")
